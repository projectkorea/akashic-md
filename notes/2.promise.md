...
- 프로미스 객체의 추상 구조는 다음과 같으며, 실제로 접근 할 수 있는 프로퍼티는 아니다.
...
// 3. finally
```
- 비동기 함수 `setTimeout`가 실행되기 전에 `reject()`가 실행되어 **rejected된 proimse가 반환된다.**
...
- **`.then()`, `.catch()`는 `Promise`를 리턴**하기 때문에 `.then() | .catch()`로 체이닝 할 수 있다.
...
##### 2) `Promise.race()`
-   `Promise.race([func1(), func2()])`: 배열에서 가장 먼저 return하는 값으로 전달한다.
