...
- `innerText`는 문서 내 노드들의 레이아웃, 포지션을 재계산 후 다시 뿌려주는 `Reflow`가 발생한다.
- `innerText`는 스타일을 알고 있기 때문에, 숨겨진 요소들의 텍스트는 반환하지 않는다. 
...
```
- As a result, `innerText` is much more performance-heavy: it requires layout information to return the result called `relfow`
