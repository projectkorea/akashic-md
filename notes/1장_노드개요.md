...
- HTML 문서를 작성할 때, HTML 콘텐츠를 다른 HTML 콘텐츠 내에 **캡슐화**하게 되며, 
  브라우저는 HTML 문서를 해석해서 트리 형태로 **구조화**, **계층화**된 노드들을 가지고 있는 `DOM` 을 생성한다.
...
- `Node`도 `Javascript` 생성자 함수이기 때문에 다른 객체들 처럼 `Object.prototype`를 상속받았다.
...
#### `innerText` (비표준) vs `textContent` (DOM 사양)
