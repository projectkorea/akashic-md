...
- **이미지가 빌드될 때** 실행할 명령어를 지정해.
- 주로 패키지 설치, 파일 복사, 소스 코드 빌드 등의 작업을 수행해.
...
- `CMD`는 기본적으로 하나의 컨테이너에 하나만 있어야 하고, 컨테이너가 시작될 때 해당 명령어가 실행돼. `CMD`는 **이미지를 빌드할 때 실행되는 게 아니라, 컨테이너가 실행될 때만 실행**돼.
- **형식**: `CMD ["실행 파일", "옵션1", "옵션2"]` 또는 `CMD <명령어>`

```
# **Node.js로 `app.js` 파일을 실행**한다는 뜻이야.
CMD ["node", "app.js"]
```
